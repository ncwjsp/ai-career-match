...
export function JobImport() {
  ...
  return <form onSubmit={submit} className="mt-8 max-w-xl space-y-5">
    <p>Paste a single job posting URL from Greenhouse, Lever or Ashby. Paste it again later to check for changes.</p>
    <div><label htmlFor="job-url" className="block font-medium">Job URL</label>
      <input id="job-url" type="url" required maxLength={2048} value={url} onChange={event => setUrl(event.target.value)} placeholder="https://jobs.lever.co/company/posting-id" className="mt-2 w-full rounded border p-3" /></div>
